module.exports = async (client, message, args) => {
    ...
    if (typeof bUser !== "string") {
        ...
        if (executorRole.comparePositionTo(targetRole) <= 0 && message.author.id !== message.guild.ownerID) {
            return message.reply("Você não pode punir este usuário, seu cargo é menor ou igual a o do usuário a ser punido! <:noswift:529635602292015134>").catch(()=>{});
        }
        let clientRole = message.guild.me.highestRole;
        if (clientRole.comparePositionTo(targetRole) <= 0) {
            return message.reply("Não posso punir este usúario, meu cargo é menor ou igual a o do usuário a ser punido! <:noswift:529635602292015134>").catch(()=>{});
        }
    }
    ...
    message.guild.ban(bUser, {reason : bReason})
        .then(async banned => {
            ...
            message.channel.send(`${user} foi banido com sucesso! <:banned:533045584542564353>`).catch(()=>{});
        })
        .catch(err => {
